...
Massachusetts's tax structure. The Tax Foundation ranks Massachusetts 37th on its State Tax Competitiveness Index.
...
How Massachusetts generates its state tax revenue, by source:

!!! info "Revenue Data Pending"
    Revenue composition data for Massachusetts will be available after the next Census data pipeline run.

## Who Pays? Effective Tax Rates by Income

!!! info "Distributional Data Pending"
    ITEP distributional data is not yet available for Massachusetts. Data collection is in progress. See [Who Actually Pays?](../analysis/who-actually-pays.md) for methodology.
