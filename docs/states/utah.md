...
Utah's tax structure. The Tax Foundation ranks Utah 10th on its State Tax Competitiveness Index.
...
How Utah generates its state tax revenue, by source:

!!! info "Revenue Data Pending"
    Revenue composition data for Utah will be available after the next Census data pipeline run.

## Who Pays? Effective Tax Rates by Income

!!! info "Distributional Data Pending"
    ITEP distributional data is not yet available for Utah. Data collection is in progress. See [Who Actually Pays?](../analysis/who-actually-pays.md) for methodology.
